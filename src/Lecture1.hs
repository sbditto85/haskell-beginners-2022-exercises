...
subString start end str =
  let
    toDrop = max 0 start
    toTake = end - toDrop + 1
    numberOfCharacters = toTake - toDrop
  in
...
lowerAndGreater n list = go n list 0 0
  where
    ...
    go needle [] countLower countHigher =
      show needle ++ " is greater than " ++ show countLower ++ " elements and lower than " ++ show countHigher ++ " elements"

    go needle (x:rest) countLower countHigher =
      if needle < x then
        go needle rest countLower (countHigher + 1)
      else if needle > x then
        go needle rest (countLower + 1) countHigher
      else
        go needle rest countLower countHigher
